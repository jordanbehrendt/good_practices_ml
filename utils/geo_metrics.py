...
Module with methods to compute the evaluation metrics
based on geographic prior (region groups).
"""
# Imports
# Built-in
import os
...
def calculate_experiment_country_accuracy(batch_df: pd.DataFrame) -> float:
    ...
    Args:
        batch_df (pd.DataFrame): DataFrame containing 'label' and
        'Predicted labels'.
    ...
    return metrics.accuracy_score(
        ...
        batch_df["Predicted labels"].tolist()
    )
...
def calculate_experiment_region_accuracy(
    country_list: pd.DataFrame,
    ...
    merged_df = pd.merge(
        batch_df,
        country_list,
        left_on='Predicted labels',
        right_on='Country',
        how='inner'
    )
    merged_df = merged_df.rename(
        columns={'Intermediate Region Name': 'predicted_region'}
    )
    ...
    merged_df = merged_df.rename(
        columns={'Intermediate Region Name': 'reference_region'}
    )
    del merged_df['Country']
    return metrics.accuracy_score(
        merged_df["reference_region"].tolist(),
        merged_df["predicted_region"].tolist()
    )
...
def calculate_region_accuracy(
    ...
    """
    index_predictions = [
        torch.argmax(prediciton) for prediciton in predictions
    ]
    region_prediciotn = [
        country_list['Intermediate Region Name'].iloc[index.item()]
        for index in index_predictions
    ]
    region_label = [
        country_list['Intermediate Region Name']
        .loc[country_list['Country'] == index]
        for index in labels
    ]
...
def calculate_metric(
    repo_path: str, batch_df: pd.DataFrame, metric_name: str
) -> float:
    """
    ...
    Args:
        ...
        batch_df (pd.DataFrame): DataFrame containing 'label' and
            'Predicted labels'.
        metric_name (str): Name of the metric to be calculated
            ('country_acc', 'region_acc' or 'mixed').
    ...
    batch_df['Probs-Array'] = batch_df['All-Probs'].apply(
        lambda x: ast.literal_eval(x)
    )
    batch_df['Predicted labels'] = batch_df['Probs-Array'].apply(
        lambda x: country_list['Country'].iloc[np.argmax(np.array(x))]
    )
...
def calculate_experiment_metric(
    repo_path: str, exp_dir: str, metric_name: str
) -> list:
    """
    ...
    Returns:
        dict: Dictionary with experiment names as keys and corresponding
            metric values as lists.
    """
    exp_metric = []
    for batch_file in os.listdir(exp_dir):
        if not os.path.isdir(os.path.join(exp_dir, batch_file)):
            ...
            exp_metric.append(
                calculate_metric(repo_path, batch_df, metric_name)
            )
    return np.array(exp_metric)


def calculate_mixed_metric(
    repo_path: str, batch_df: pd.DataFrame
):
    country_list = pd.read_csv(
        f'{repo_path}/utils/country_list/country_list_region_and_continent.csv'
    )
    merged_df = pd.merge(
        batch_df, country_list,
        left_on='Predicted labels',
        right_on='Country',
        how='inner'
    )
    ...
    unique_countries = pd.concat(
        merged_df['label'], merged_df['Predicted labels']
    ).unique().tolist()
    ...
    for idx, country in enumerate(unique_countries):
        true_pos = np.array(
            [x == country for x in merged_df['label'].tolist()]
        )
        pred_pos = np.array(
            [x == country for x in merged_df['Predicted labels'].tolist()]
        )
        region = country_list\
            .loc[
        ...
        true_region = np.array(
            [x == region for x in merged_df['reference_region'].tolist()]
        )
        pred_region = np.array(
            [x == region for x in merged_df['predicted_region'].tolist()]
        )

        TP[idx] = np.sum(
            (true_pos & pred_pos).sum(),
            .5 * (
                (true_pos & ~pred_pos) & (pred_region & true_region)
            ).sum()
        )
        FP[idx] = (~true_pos & pred_pos).sum()
        FN[idx] = (true_pos & ~pred_pos & ~pred_region).sum()
