# -*- coding: utf-8 -*-
"""
CLIP_Experiment.run_statistical_tests
-------------------------------------

Script to run statistical tests to compare
baseline CLIP performance using different
...
def compute_paired_k_fold_cross_validation_t_test(
        results_1: list, results_2: list, k: int, r: int
):
    """
    Compute paired k fold cross validation t-test in accordance with the
    method described in
    ...
    Args:
        ...
        results_2 (list): List of accuracies for each batch in second experiment  # noqa: E501
    ...
    if num_experiments != len(results_2):
        raise Exception('The results are not the same length.')
    if num_experiments != r * k:
        raise Exception('The results do not coincide with the r and k values.')
...
def run_analysis_of_dataset_and_prompts(
    output_dir: str,
    comparison_df: pd.DataFrame,
    metric: str, paired_ttest: bool
) -> None:
    """
    Save metrics DataFrame to a CSV file and calculate statistics
    (median, mean, std deviation) for each model.
    ...
    for i in range(num_datasets):
        ...
        for j in range(num_datasets):
            if i > j:
                if paired_ttest:
                    t, p, df = compute_paired_k_fold_cross_validation_t_test(
                        comparison_df.iloc[:, i].tolist(),
                        comparison_df.iloc[:, j].tolist(),
                        20, 10
                    )
                    t_values.append(t)
                    p_values.append(p)
                    ...
                    df_values.append(
                        len(comparison_df.iloc[:, i].tolist()) - 1
                    )
            else:
                t_values.append(0.0)
                p_values.append(0.0)
...
def run_analysis_of_experiments(REPO_PATH, metric):
    ...
    for dataset in datasets:
        default_experiment = pd.read_csv(
            f"{REPO_PATH}/CLIP_Experiment/result_accuracy/"
            f"{metric}/default_prompt/{dataset}.csv"
        )
        extended_prompt_experiment = pd.read_csv(
            f"{REPO_PATH}/CLIP_Experiment/result_accuracy/"
            f"{metric}/extended_prompt/{dataset}.csv"
        )
        default_list = []
        ...
        comparison_df = pd.DataFrame({
            'default_prompt': default_list,
            'extended_prompt': extended_prompt_list
        })
        default_df[f'{dataset}'] = default_list
        extended_prompt_df[f'{dataset}'] = extended_prompt_list
        output_dir = f'{REPO_PATH}/CLIP_Experiment/statistical_tests/{metric}/{dataset}/'  # noqa: E501
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        run_analysis_of_dataset_and_prompts(
            output_dir, comparison_df, metric, True
        )

    default_output_dir = f'{REPO_PATH}/CLIP_Experiment/statistical_tests/{metric}/default_prompt/'  # noqa: E501
    extended_prompt_output_dir = f'{REPO_PATH}/CLIP_Experiment/statistical_tests/{metric}/extended_prompt/'  # noqa: E501
    if not os.path.exists(default_output_dir):
        os.makedirs(default_output_dir)
    if not os.path.exists(extended_prompt_output_dir):
        os.makedirs(extended_prompt_output_dir)
    run_analysis_of_dataset_and_prompts(
        default_output_dir, default_df, metric, False
    )
    run_analysis_of_dataset_and_prompts(
        extended_prompt_output_dir, extended_prompt_df, metric, False
    )
