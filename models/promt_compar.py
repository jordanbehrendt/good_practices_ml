...
def run_experiment(DATA_PATH: str, debug: bool, config_path: str):
    ...
    country_list = pd.read_csv(os.path.join(REPO_PATH,"data_exploration","image_distribution.csv"))['label'].to_list()
    ...
    standard_tester = testing.ModelTester(test_dataset, model, (lambda x : x), batch_size, country_list, seed, REPO_PATH, 'pretrained','no_prompt','experiment0')
    standard_tester.run_test()
    elab_tester = testing.ModelTester(test_dataset, model, (lambda x : f"This image shows the country {x}"), batch_size, country_list, seed, REPO_PATH, 'pretrained','elab_prompt','experiment0')
    elab_tester.run_test()
    streetveiw_tester = testing.ModelTester(test_dataset, model,(lambda x : f"A google streetview image from {x}"), batch_size, country_list, seed, REPO_PATH, 'pretrained','streetveiw_prompt','experiment0')
