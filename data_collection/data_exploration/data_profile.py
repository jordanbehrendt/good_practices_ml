# -*- coding: utf-8 -*-
"""
data_exploration.data_profile
-----------------------------

Script to profile data distribution of a given dataset.
"""
# Imports
# Built-in
import os
...
def line_graph(
    ...
    Args:
        image_distribution_path (str): File path of the image distribution
            data (CSV format).
    ...
    if logarithmic:
        plt.plot(
            image_distribution['label'],
            image_distribution['count'].apply(log),
            linestyle='-',
            color='b'
        )
    ...
    plt.xticks(
        range(len(image_distribution['label'])),
        empty_labels,
        fontsize='small'
    )
...
def world_heat_map(
    ...
    data_frame_full = (
        world
        .merge(image_distribution, on='join')
        .drop('join', axis=1)
    )
    image_distribution.drop('join', axis=1, inplace=True)

    # Checking for matches between 'name' and 'label' columns
    data_frame_full['match'] = data_frame_full.apply(
        lambda x: x["name"].find(x["label"]),
        axis=1
    ).ge(0)
...
def data_profile(dataset_dir: str, REPO_PATH: str, dataset_name: str) -> None:
    """
    Generates a profile report, image distribution CSV, world heat map, and
    line graph based on a dataset.
    ...
    output_dir = os.path.join(
        REPO_PATH,
        'data_collection/data_exploration',
        dataset_name
    )
    ...
    image_distribution.to_csv(
        os.path.join(output_dir, 'image_distribution.csv')
    )

    # Generating and saving world heat map and line graph based on image
    # distribution (default and logarithmic) and save it in one pdf
    world_heat_map(
        os.path.join(output_dir, 'image_distribution.csv'),
...
def create_dataset_profile(yaml_path: str, dataset_name: str) -> None:
    """
    Create a dataset profile including a report, image distribution CSV,
    and world heat map.
