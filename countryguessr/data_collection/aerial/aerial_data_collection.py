# -*- coding: utf-8 -*-
"""
aerial.aerial_data_collection
aerial-----------------------

Script to scrape AERIAL dataset images.
"""
# Imports
# Built-in
import os
...
def find_interior_boxes(REPO_PATH):
    """Find a large interior box for each country, based on
    polygon data taken from natural earth
    (https://github.com/martynafford/natural-earth-geojson/blob/master/110m/cultural/ne_110m_admin_0_countries.json)  # noqa: E501
    ...
    """
    polygon_file = open(
        f"{REPO_PATH}/data_collection/aerial/ne_110m_admin_0_countries.json"
    )
    polygons_json = json.load(polygon_file)
    countries_polygons = polygons_json["features"]

    country_panda = pandas.read_csv(
    ...
    for index, row in country_panda.iterrows():
            ...
            for el in polygon_object:
                if len(el) > 1:
                    ...
                    for nested_point in el[0]:
                        nested.append((
                            float(nested_point[0]),
                            float(nested_point[1])
                        ))
        ...
        if (len(coords) > 0):
            ...
            if result.covers(box):
                ...
                interior_boxes.append({
                    'Country': row['Country'],
                    'Alpha2Code': row['Alpha2Code'],
                    'box': [min(xx), max(yy), max(xx), min(yy)],
                    'boxes': ''
                })
            else:
                print(row['Country'])
    ...
    interior_panda.to_csv(
        f"{REPO_PATH}/data_collection/aerial/interior_boxes.csv"
    )


def get_rectangle(polygon, center):
    """Creates rectangle based on the given polygon and the center point.
    First measures the shortest distances to the boundaries in the x and y
    directions then creates and returns a rectangle defined by these
    smallest distances.
    ...
    Returns:
        bbox (List): The largest possible rectangle (probably not interior)
            created by going as far as possible in x and y directions
    """
    distances = []
    multipoints = []
    points = []
    boundary = polygon.boundary
    multipoints.append(boundary.intersection(
        LineString([(center.x + 500, center.y), (center.x - 500, center.y)]),
        grid_size=1E-14
    ))
    multipoints.append(boundary.intersection(
        LineString([(center.x, center.y + 500), (center.x, center.y - 500)]),
        grid_size=1E-14
    ))
    for i in range(0, len(multipoints)):
        ...
        points.append(intersections[
            intersections_distances.index(min(intersections_distances))
        ])
    dif_x = points[0].x - center.x
    dif_y = points[1].y - center.y
    bbox = [
        [center.x - dif_x, center.y + dif_y],
        [center.x + dif_x, center.y + dif_y],
        [center.x + dif_x, center.y - dif_y],
        [center.x - dif_x, center.y + dif_y]
    ]
...
def get_aerial_images(REPO_PATH, DATA_PATH):
    """Finds images from openaerialmap.org using interior_boxes saved
    in interior_boxes.csv and bounding boxes (for a few island or
    peninsular countries) saved in island_bounding_boxes.csv
    ...
    Returns:
        None
    """
    interior_boxes = pandas.read_csv(
        f'{REPO_PATH}/data_collection/aerial/interior_boxes.csv'
    )

    for _, row in interior_boxes.iterrows():
        count = 0
        if not pandas.isnull(row["box"]):
            new_count = get_images_from_bbox(
                ast.literal_eval(row["box"]),
                count,
                row["Country"],
                DATA_PATH
            )
    ...
    island_bounding_boxes = pandas.read_csv(
        f'{REPO_PATH}/data_collection/aerial/island_bounding_boxes.csv'
    )
    for _, row in island_bounding_boxes.iterrows():
        get_images_from_bbox(
            [row['x1'], row['y1'], row['x2'], row['y2']],
            0,
            f"islandbbox-{row['Country']}",
            DATA_PATH
        )


def get_images_from_bbox(bbox, count, country, DATA_PATH):
    """Downloads images within the given bounding box from openaerialmap
    using the leafmap module

    Args:
        bbox (List): The bounding box that provides the area within which
            images can be found
        count (int): The current amount of images already found
        country (str): the name of the country (prefaced with islandbbox- for
            island countries) to provide the folder and path for saving of
            the images
        DATA_PATH (str): Path to data folder.
    ...
    if gdf is not None:
        ...
        if len(images) != 0:
            ...
            os.makedirs(f"{DATA_PATH}/aerial/{only_country}/", exist_ok=True)
            for i in range(0, min(20, len(images))):
                img_data = requests.get(images[i]).content
                with open(
                    f"{DATA_PATH}/aerial/{only_country}/"
                    f"aerial-{country}-{count}.png",
                    'wb'
                ) as handler:
