# -*- coding: utf-8 -*-
"""
finetuning.create_datasets_from_embeddings
-----------------------------------------

Script to generate training and testing splits from stored embeddings.
"""
# Imports
# Built-in
import os
...
def balance_data(
    df: pd.DataFrame, max_images: int = 1000,
    min_images: int = 10, seed: int = 1234
):
    """
    Balance the data in a DataFrame by randomly sampling a maximum of images
    from each class, dropping classes below the minimum.

    Args:
        df (pd.DataFrame): The DataFrame containing the data.
        REPO_PATH (str): The path to the repository.
        max_images (int, optional): The maximum number of images to sample.
            Defaults to 1000.
        min_images (int, optional): The minimum number of images to sample.
            Defaults to 10.
...
def create_datasets_from_embeddings(
    ...
    Args:
        ...
        seed (int, optional): The random seed for reproducibility.
            Defaults to 1234.
        min_geo_percentage (float, optional): The minimum percentage of geo
            images to replace with aerial and tourist images. Defaults to 0.5.
        max_images_weakly (int, optional): The maximum number of images for
            weakly balanced datasets. Defaults to 2000.
        max_images_strongly (int, optional): The maximum number of images for
            strongly balanced datasets. Defaults to 200.
        min_geo_images (int, optional): The minimum number of geo images.
            Defaults to 10.
        min_aerial_images (int, optional): The minimum number of aerial
            images. Defaults to 2.
        min_tourist_images (int, optional): The minimum number of tourist
            images. Defaults to 10.
        test_size (float, optional): The proportion of the dataset to include
            in the test split. Defaults to 0.15.
    """
    ...
    with torch.no_grad():
        ...
        geo_embed = pd.read_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Image/geoguessr_embeddings.csv"
            )
        )
        aerial_df = pd.read_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Image/aerial_embeddings.csv"
            )
        )
        tourist_df = pd.read_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Image/tourist_embeddings.csv"
            )
        )
        ...
        # Make sure that all labels in the training set are also
        # in the test set
        labels_in_aerial_train_not_in_aerial_test = (
            set(
                aerial_train_and_val['label']
                .value_counts()
                .keys()
                .to_list()
            ) - set(
        ...
        geo_zero_shot_df = geo_embed[
            ~geo_embed["label"].isin(test_data["label"])
        ]
        ...
        test_data.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Testing/known_test_data.csv"
            ),
            index=False,
        )
        zero_shot_data.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Testing/zero_shot_test_data.csv"
            ),
            index=False,
        )
        ...
        weakly_balanced_geo_df.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Training/geo_weakly_balanced.csv"
            ),
            index=False,
        )

        # Get all images from geo_df for the classes that have more than
        # 2000 images and that are not in balanced_geo_df
        geo_large_classes = (
            geo_embed["label"]
            .value_counts()[
                geo_embed["label"].value_counts() > max_images_weakly]
            .index.tolist()
        )
        geo_additional_images = geo_embed[
            geo_embed["label"].isin(geo_large_classes) &  # noqa: W504
        ...
        unbalanced_geo_df.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Training/geo_unbalanced.csv"
            ),
            index=False,
        )

        # Remove images of classes to have a maximum of 200 images for
        # the strongly balanced dataset
        ...
        strongley_balanced_geo_df.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Training/geo_strongly_balanced.csv"
            ),
            index=False,
        )

        # Replace up to 50% of each label with images of that same label
        # from aerial and tourist data in the weakly balanced set
        ...
        for label in mixed_weakly_balanced_df["label"].unique():
            ...
            aerial_images = aerial_train_and_val[
                aerial_train_and_val["label"] == label
            ]
            ...
            replace_indices = (
                mixed_weakly_balanced_df[
                    mixed_weakly_balanced_df["label"] == label]
                .sample(num_replace, random_state=seed)
                .index
            )
            cols_to_use = mixed_weakly_balanced_df.columns
            mixed_weakly_balanced_df.loc[replace_indices] = (
                replace_images[cols_to_use]
                .sample(num_replace, random_state=seed)
                .to_numpy()
            )

        # Replace up to 50% of each label with images of that same label
        # from aerial and tourist data in the strongly balanced set
        ...
        for label in mixed_strongly_balanced_df["label"].unique():
            ...
            replace_images = pd.concat([aerial_images, tourist_images])
            replace_indices = (
                mixed_strongly_balanced_df[
                    mixed_strongly_balanced_df["label"] == label]
                .sample(num_replace, random_state=seed)
                .index
            )
            cols_to_use = mixed_strongly_balanced_df.columns
            mixed_strongly_balanced_df.loc[replace_indices] = (
                replace_images[cols_to_use]
                .sample(num_replace, random_state=seed)
                .to_numpy()
            )
        ...
        mixed_weakly_balanced_df.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Training/mixed_weakly_balanced.csv"
            ),
            index=False,
        )
        mixed_strongly_balanced_df.to_csv(
            os.path.join(
                REPO_PATH,
                "CLIP_Embeddings/Training/mixed_strongly_balanced.csv"
            ),
            index=False,
        )
        ...
        # Check how much data from the tourist df is in the mixed
        # weakly dataframe
        tourist_in_mixed_weakly = mixed_weakly_balanced_df[
            mixed_weakly_balanced_df["path"].isin(tourist_df["path"])
        ]
        tourist_count_in_mixed_weakly = len(tourist_in_mixed_weakly)
        print(
            "Number of tourist data in mixed weakly balanced dataset: "
            f"{tourist_count_in_mixed_weakly}"
        )

        # Check how much data from the aerial df is in the mixed
        # weakly dataframe
        aerial_in_mixed_weakly = mixed_weakly_balanced_df[
            mixed_weakly_balanced_df["path"].isin(aerial_df["path"])
        ]
        aerial_count_in_mixed_weakly = len(aerial_in_mixed_weakly)
        print(
            "Number of aerial data in mixed weakly balanced dataset: "
            f"{aerial_count_in_mixed_weakly}"
        )

        # Check how much data from the tourist df is in the mixed
        # strongly dataframe
        tourist_in_mixed_strongly = mixed_strongly_balanced_df[
            mixed_strongly_balanced_df["path"].isin(tourist_df["path"])
        ]
        tourist_count_in_mixed_strongly = len(tourist_in_mixed_strongly)
        print(
            "Number of tourist data in mixed strongly balanced dataset: "
            f"{tourist_count_in_mixed_strongly}"
        )

        # Check how much data from the aerial df is in the mixed
        # strongly dataframe
        aerial_in_mixed_strongly = mixed_strongly_balanced_df[
            mixed_strongly_balanced_df["path"].isin(aerial_df["path"])
        ]
        aerial_count_in_mixed_strongly = len(aerial_in_mixed_strongly)
        print(
            "Number of aerial data in mixed strongly balanced dataset: "
            f"{aerial_count_in_mixed_strongly}"
        )
        ...
        tourist_in_zero_shot = zero_shot_data[
            zero_shot_data["path"].isin(tourist_df["path"])
        ]
        tourist_count_in_zero_shot = len(tourist_in_zero_shot)
        print(
            "Number of tourist data in zero shot dataset: "
            f"{tourist_count_in_zero_shot}"
        )

        # Check how much data from the aerial df is in the zero shot data
        aerial_in_zero_shot = zero_shot_data[
            zero_shot_data["path"].isin(aerial_df["path"])
        ]
        aerial_count_in_zero_shot = len(aerial_in_zero_shot)
        print(
            "Number of aerial data in zero shot dataset: "
            f"{aerial_count_in_zero_shot}"
        )


if __name__ == "__main__":
    """Creates the test set and the different train/valdiation sets.
    This script uses the model inputs created using the
    ...
    parser.add_argument(
        ...
        type=float,
        help=(
            "The minimum percentage of geo images to replace "
            "with aerial and tourist images"
        )
    )
    parser.add_argument(
        "--max_images_weakly",
    ...
    create_datasets_from_embeddings(args.repo_path)
